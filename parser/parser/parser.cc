...
void NR_Iterations(double jacMat[][30], double result[], double minDert[], int number, int& count, double accurateValue, int datum, int lastnode);
    ...
    //------------------------同伦法求解电路方程
        ...
        while (t <=1.0) {
            ...
            if (count < 5) {
                step = step * 2;
                for (int i = 0; i < number; i++) {
                    PnodeValue[i + 1] = nodeValue[i + 1];//存储最优结果
                }
            }
